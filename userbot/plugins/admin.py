...
NO_ADMIN = "`I am not an admin! Chutiya Noob nibbe sala`"
...
async def promote(promt):
    ...
    try:
        ...
        await hellevent.edit("`Promoted Successfully! Abb nach le bosdike💃🕺`")
...
async def demote(dmod):
    ...
    try:
        ...
        await hellevent.edit(NO_PERM)
        return
    await hellevent.edit("`Demoted retard Successfully..... Better luck next time Bosdike🚶`")
    if BOTLOG:
...
async def ban(bon):
    ...
    if reason:
        ...
        await hellevent.edit(f"{str(user.id)} Madarchod is banned😏 !!")
    if BOTLOG:
