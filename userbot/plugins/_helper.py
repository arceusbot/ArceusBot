...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Aceus User"
            ...
            if len(string) > 4095:
                with io.BytesIO(str.encode(string)) as out_file:
                    ...
                    await bot.send_file(
                        ...
                        force_document=True,
                        allow_cache=False,
                        caption="**COMMANDS** In __**arceusbot**__",
...
async def _(event):
    ...
    await event.edit("""Telethon UserBot Powered by @ArceusTheuserBot""")
...
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of Arceusbot\n\n"
        )
        hellcount = 0
        plugincount = 0
        for i in sorted(SUDO_LIST):
            ...
            string += "\n"
        if len(string) > 4095:
            data = string.format(count=arcount, plugincount=plugincount)
            key = (
                requests.post(
            ...
            reply_text = f"All commands of the arceusbot are [here]({url})"
            await event.reply(reply_text, link_preview=False)
            return
        await event.reply(
            string.format(count=arcount, plugincount=plugincount), link_preview=False
        )
        return
    if input_str:
        if input_str in SUDO_LIST:
            ...
            for i in SUDO_LIST[input_str]:
                ...
                hellcount += 1
            await event.reply(
                string.format(count=arcount, input_str=input_str), parse_mode="HTML"
            )
        else:
        ...
        for i in sorted(SUDO_LIST):
            ...
            string += " "
            arceuscount += 1
        await event.reply(string.format(count=arcount), parse_mode="HTML")
